...
- **Fase 2 — Agenda** (siguiente): migración `agenda` (Especialidad, Consultorio, Cita con constraint de exclusión anti-solape, Recordatorio stub), vistas día/semana/mes, máquina de estados de cita. Construir `SearchSelect` aquí (buscar paciente al crear cita).
- Fases 3-10 según el plan: Admisiones → HCE → Fórmulas → Facturación/Caja → RIPS → Parametrización/Sedes → Roles/Auditoría/listado Usuarios → Dashboard/Reportes/Configuración.
